 ...
 * @module	codeQuake/WCF/Language/MultipleLanguageWYSIWYG
 */
define(['Core', 'Dictionary', 'Language', 'ObjectMap', 'StringUtil', 'Dom/Traverse', 'Dom/Util', 'Ui/SimpleDropdown', 'EventHandler'], function(Core, Dictionary, Language, ObjectMap, StringUtil, DomTraverse, DomUtil, UiSimpleDropdown, EventHandler) {
	"use strict";
	...
	var MultipleLanguageWYSIWYG = {
		...
		_initElement: function(elementId, element, values, availableLanguages, forceSelection) {
			...
			if (!container.classList.contains('inputAddon')) {
				...
				container.className = 'inputAddon inputAddonTextarea';
				elData(container, 'input-id', elementId);
				container.appendChild(element);
				redactorContainer.parentNode.insertBefore(container, redactorContainer);
			}
		...
		_select: function(elementId, languageId, isInit) {
			...
			if (data.languageId !== languageId) {
				...
				if (data.languageId) {
					values.set(data.languageId, redactor.redactor('code.get'));
				}
				...
				if (languageId === 0) {
					...
					redactor.redactor('code.set', (values.has(languageId)) ? values.get(languageId) : '');
				}
	...
	return MultipleLanguageWYSIWYG;
});
